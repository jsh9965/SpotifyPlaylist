...
### One Hit Wonders
Once the user selects a playlist, the app looks for artists that have exactly one song on that playlist.  These songs are considered the user's 'One Hit Wonders' as they are the only song by that artist that the user listens to.  The analysis only considers the first artist listed on any given track, as it assumes that they are the primary artist.  The app can then optionally create a new playlist containing just their 'One Hit Wonders'.  The main use case for this is finding artists you like, but haven't explored much and therefore might be ripe for further listening.
...
## How to run
To run this app, you must have a Spotify developer account to use the Spotify web API.  Once you do, fill in the relevant client_id and client_secrets in the .env file, as well as redirect_uri, which you have whitelisted on Spotify, and a secret to use for signatures (delete '.example' once you are done).  Then, use pip or pipenv to install the dependencies listed in the pipfile, and run app.py.  This will host the app according to app.py, which is set to use localhost by default.  The console will output the relevant URL, which can be visited to see the app.  Any Spotify accounts that attempt to sign in must be whitelisted on the Spotify developers dashboard.
